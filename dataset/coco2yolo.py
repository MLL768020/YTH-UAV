...
coco_format_save_path = '/home/cv/lyh/datasets/RFRB/annotations'  # 要生成的标准coco格式标签所在文件夹
yolo_format_classes_path = '/home/cv/lyh/datasets/RFRB/val/classes.txt'  # 类别文件，一行一个类
yolo_format_annotation_path = '/home/cv/lyh/datasets/RFRB/val/labels'  # yolo格式标签所在文件夹
img_pathDir = '/home/cv/lyh/datasets/RFRB/val/images'  # 图片所在文件夹
...
for j, label in enumerate(lines1):
    label = label.strip()
    categories.append({'id': j + 1, 'name': label, 'supercategory': 'Nodule'})  # 将类别信息添加到categories中
# print(categories)
...
for i, imageFile in enumerate(imageFileList):
    ...
    # 获取该图片获取的txt文件
    if os.path.exists(os.path.join(yolo_format_annotation_path, txtFile)):
        ...
        for j, line in enumerate(lines):
            ...
            # #注意目标类别要加一
            # category = {}
            # category['category_id']=class_id+1
            # category['name'] = 'Nodule'
            # bbox_dict['category'] = category
            bbox_dict['iscrowd'] = 0
...
name = os.path.join(coco_format_save_path, "val_6354" + '.json')
