...
pwd = os.getcwd()
# giou eiou siou smi
# names = ['ncv8lrtdetryuan2','nctrain1v8lrtdetr3','ncv8lrtdetrs','ncv8lrtdetr3new']
names = ['RTYO','YH-RTYO','YOLOv8-L','RTDETR-L']

plt.figure(figsize=(10, 10))

plt.subplot(2, 2, 1)
for i in names:
    data = pd.read_csv(f'runs/llf/{i}/results.csv')
...
for i in names:
    data = pd.read_csv(f'runs/llf/{i}/results.csv')
    data['        train/giou_loss'] = data['        train/giou_loss'].astype(float).replace(np.inf, np.nan)
...
for i in names:
    data = pd.read_csv(f'runs/llf/{i}/results.csv')
    data['          train/l1_loss'] = data['          train/l1_loss'].astype(float).replace(np.inf, np.nan)
...
for i in names:
    data = pd.read_csv(f'runs/llf/{i}/results.csv')
    data['          val/giou_loss'] = data['          val/giou_loss'].astype(float).replace(np.inf, np.nan)
...
for i in names:
    data = pd.read_csv(f'runs/llf/{i}/results.csv')
    data['            val/l1_loss'] = data['            val/l1_loss'].astype(float).replace(np.inf, np.nan)
